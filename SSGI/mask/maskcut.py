...
def create_annotation_info(img_name ,annotation_id, image_id, category_info, binary_mask, 
    ...
    #print("binary_mask:", binary_mask_encoded)      
    
    binary_mask_encoded = mask.encode(np.asfortranarray(binary_mask.astype(np.uint8)))
    
    #print("binary_mask_encoded:", binary_mask_encoded)
    ...
    for contour in contours:
        for point in contour:
            x, y = point[0]
            contour_coordinates.extend([x, y])
    contour_coordinates = [int(x) for x in contour_coordinates]
    ...
    # 绘制轮廓
    cv2.drawContours(blank_image, contours, -1, (0, 255, 0), 2)  # 使用绿色绘制轮廓

    # 设置保存的文件路径
    save_path = '../output/contours_image.png'
    ...
    #non_zero_coordinates = np.transpose(np.nonzero(decoded_mask))
    #coordinates = non_zero_coordinates[:, 1].tolist() + non_zero_coordinates[:, 0].tolist()

    #print("decoded_mask:", contour_coordinates)

    segmentation = contour_coordinates
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset-path', type=str, default="", help='path to the dataset') #imagenet/train/
    ...
    if args.vit_arch == 'base' and args.patch_size == 8:
        if args.pretrain_path is None:
            file_path = "https://dl.fbaipublicfiles.com/dino/dino_vitbase8_pretrain/dino_vitbase8_pretrain.pth"#"https://dl.fbaipublicfiles.com/dino/dino_vitbase8_pretrain/dino_vitbase8_pretrain.pth"
        feat_dim = 768
    elif args.vit_arch == 'small' and args.patch_size == 8:
        if args.pretrain_path is None:
            file_path = "https://dl.fbaipublicfiles.com/dino/dino_deitsmall8_300ep_pretrain/dino_deitsmall8_300ep_pretrain.pth"
        feat_dim = 384

    backbone = dino.ViTFeat(file_path, feat_dim, args.vit_arch, args.vit_feat, args.patch_size)
        ...
        for img_name in tqdm(img_list) :
            ...
            for idx, bipartition in enumerate(bipartitions):
                ...
                annotation_info = create_annotation_info(
                    img_name ,segmentation_id, image_id, category_info, pseudo_mask.astype(np.uint8), None)
